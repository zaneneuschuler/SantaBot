    ...
    if str(ctx.author.id) in os.getenv("ADMIN_IDS"):  # TODO: also put this in .env
        ...
        for i in range(len(stepartists)):
            ...
            new_file = shutil.copyfile(
                f"uploads/{lucky_boy_or_girl}.png",
                f"uploads/{stepartists[i][1]}present.png",
...
async def help(ctx):
    ...
    embed.add_field(
        ...
        value="Unfortunately, at this time **SantaBot** is not able to accept your final submissions, so that will still be handled by sending your file to <@262440960040894474>, however in the future we can try to automate that and the pack creation process as well!",
        inline=True,
    )
    embed.add_field(
